...
from stl2g.model.STL2G import STL2G


def STL2G_prepare_training(d_model_dict, head_dict ,d_ff, n_layers, spatial_local_dict, temporal_local_dict,
                  dropout, clf_class, domain_class, lr):
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    model = STL2G(d_model_dict, head_dict ,d_ff, n_layers, spatial_local_dict, temporal_local_dict,
                  dropout, clf_class, domain_class).to(device)
...
def subject_independent_validation(dataSet, subjects, d_model_dict, head_dict ,d_ff, n_layers, spatial_local_dict, temporal_local_dict,
    ...
    for i, (train_idx, test_idx) in enumerate(loo.split(subidx)):
        ...
        model, optimizer, lr_scheduler, criterion, device, criterion_domain = STL2G_prepare_training(d_model_dict, head_dict ,d_ff,
                                                                                                      n_layers, spatial_region_split, temporal_local_dict, dropout, clf_class, domain_class, lr)
        ...
        torch.save(best_model.state_dict(),
                   f'checkpoints/exp_{model_name}_dropout:{dropout}_attdim:{d_model_dict}_encoder_num:{n_layers}_lr:{lr}_fold:{i}.pth')
...
if __name__ == '__main__':
    ...
    temporal_local_dict = CONSTANT[dataSet]['temporal_ch_group']
    ...
    for directory in [ckpt_path]:
        if not os.path.exists(directory):
            os.makedirs(directory)
    outputfile = open(
        f'{log_path}/exp_{model_type}_numsubjects_{domain_class}_dropout{dropout}_numch{num_ch}_d_model_dict{d_model_dict}_dff{d_ff}.txt', 'w')
    sys.stdout = outputfile
    loo = LeaveOneOut()
    subject_independent_validation(dataSet, domain_class, d_model_dict, head_dict ,d_ff, n_layers, spatial_local_dict, temporal_local_dict,
